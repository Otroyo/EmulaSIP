...
uhost = "http://otroyo.pythonanywhere.com"
cpuesta = "siu<=p"
...
def inicio(clave):
    if not clave == cpuesta:
        return redirect('')
    numerorandom = str(randint(9000, 99999))
    ...
    numsips = len(sips)
    numpdfs = len(pdfs)
    return render_template("presentasips.html",
                           numsips = numsips, numpdfs = numpdfs,
                           title = 'Inicio', sips = sips, pdfs = pdfs,
                           uborrasip = uborrasip, usubesip = usubesip, usale = usale,
                           ucorresip = ucorresip, numerorandom = numerorandom, clave = clave)
